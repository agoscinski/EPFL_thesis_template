...
\begin{align}
    ...
    \int_{SO(3)} \mathrm{d}\hat{R}\, \delta(\hat{R}\mathbf{r}-\mathbf{r}_{ji}) &\propto_r \delta(r-r_{ji})\quad\textrm{(order 1)},  \\% = f^1(r)\\
    ...
    \int_{SO(3)} \mathrm{d}\hat{R}\, \delta(\hat{R}\mathbf{r}-\mathbf{r}_{ji}) \delta(\hat{R}\mathbf{r}^\prime-\mathbf{r}_{ki}) &\propto_r \delta(r-r_{ji})\delta(r^\prime-r_{ki})\delta(r_-\theta_{jki})\quad\textrm{(order 2)}. %= f^2(r, r, \theta)
...
\end{align}
\end{subequations}
Incorperating the radial part back into the above solution we obtain
\begin{equation}
  \label{eq:soap}
...
\end{align}
\end{subequations}
with various strategies for compressing the basis coefficients in high-dimensional space\cite{kondor2018clebsch,yan2019fourier,nigam2020recursive}.
...
One further includes the chemical type/species $a_j$ of atom $j$ into the the coefficients by separate the neighbor contributions in different species channels
\begin{equation}
