...
\label{sec:mlip}
%\markboth{Implementation of short-range machine learning interatomic potentials}{Implementation of short-range machine learning interatomic potentials}
%\addcontentsline{toc}{chapter}{Implementation of short-range machine learning interatomic potentials}
...
In this chapter, we discuss my contributions to the software ecosystem that facilitate the deployment of MLIPs into a MD software.
...
% and the \texttt{scikit-matter}~\cite{goscinski2023scikit} package -- a toolkit for various data-driven preprocessing methods that facilitate the MLIP construction. % as feature and sample selection.
...
%the creation of a model the shipping to a MD software.
...
One of the main contributions to \texttt{librascal} has been the implementation of a cubic spline to interpolate the radial expansion coefficients as expressed in Equation~\eqref{eq:radial_expansion}.
...
%the computation analytically the integral for the radial expansion in Equation~\eqref{eq:radial_part_in_spherical_expansion}. 
%However, due to the modified spherical Bessel function $i_l$ %in Equation~\eqref{eq:radial_part_in_spherical_expansion}
...
%\end{multline}
%The contribution of this function to the overall cost of the radial expansion can be seen in Figure~\ref{fig:hyp1f1_contribution} ranging from 55\% to 75\%  of the total time.F
...
%as it can be seen in Figure~\ref{TODO} with experiments on the chemical shieldings\cite{TOOD}.
...
%It is evident from Equation~\ref{eq:forces_interatomic_potential} that the forces acting on atom $k$ solely depend on the derivative of the local energies within its neighborhood (TODO elaborate in one more sentence wh).
...
The abundance of ML packages available today~\cite{Haddad2023Artificial}, such as \texttt{scikit-learn}~\cite{pedregosa2011scikit},\linebreak \texttt{PyTorch}~\cite{NEURIPS2019_9015}, prompts the question of whether these developments are needed. %we need to develop a new ML package.
...
%Unlike in Equation~\ref{eq:efficient_gradient_eval} the $\alpha_t$ has been extracted out to see more clearly that the loss function can be translated into the vectorial form
%\begin{subequations}
%  \begin{gather}
%    \ell(\boldsymbol{\alpha}) = \|\mathbf{K}\boldsymbol{\alpha} - \mathbf{y}\|^2, \\
...
%Note that $\mathbf{K}$ is in this case not symmetric and therefore cannot be solved the usual way as shown in Equation~\ref{eq:kernel_symmetry}.
%One can still derive a solution from Equation~\ref{eq:solving_kernel} as it is done for linear regression,
%\begin{equation}
...
%embedding the gradient information as derived in Equation~\eqref{eq:kernel_forces} to the training points used for the energy prediction in Equation~\ref{eq:kernel_evaluatio_ID1} resulting in the expression
\begin{equation}
...
%Other approaches further embedd the gradient training points as the expression in Equation~\ref{TODO} indicates into the kernel matrix and retrieve a squarte matrix~\cite{...}.
...
%It was advantegeous in Equation~\ref{eq:kernel_forces} to extract the $\alpha$ out of the sum to obtain a vectorial form
...
%Starting from the expression in Equation~\eqref{eq:force_eval_pseudo_points}
...
%A schematic of the interwork between the software packages can be seen in Figure~\ref{fig:ipi-librascal-plumed}.
%\begin{figure}
%    \includegraphics[width=\textwidth]{fig/ipi-librascal-plumed.pdf}
...
%Note that a selectional computation of partial gradients also needs to consider dependencies of the gradient on the central energies of its neighbors as pointed out in Equation~\eqref{eq:forces_interatomic_potential}.
...
%My contributions to this work was firstly, to participate in the development of the MLIP package \texttt{librascal} that was used to conduct DFT simulations at higher levels-of-theory to analyze its effect on the temperature.
...
%Note that one can simplify the gradients for further usage by summing neighbor and central contributions
%\begin{subequations}
%\begin{align}
...
%So far MLIP implementations bundle the featurization and model into one package that makes it hard to deploy different models using the same featurization.
%This lack of modularity can be attributed to the inclusion of gradients into the learning algorithm making the model more dependent on the exact form of the featurization.
%A new approach based on a abstract data type that inherently connects features and gradients by labeled metadata enables the creation of a more modular software ecosystem for the construction of MLIPs.
...
%a common data type is required that the features and model operate on.
%In the \texttt{Python} ecosystem the \texttt{numpy} array has become a widely-used data type for ML packages due to its extensive support for mathematical libraries~\cite{harris2020array}.
%The rise of data types with extended cabalities, such as automatic differentiation and GPU-based computation, has motivated the development of an API that allows these ML packages to operate on a much more extensive range of array data types~\cite{array-api}.
%The existing data types, however, lack the capability to describe structurally sparse data in combination with gradients, which is fundamental in atomistic learning due to the sparsity induced by chemical species and symmetries and its application as MLIP.
%While the package \texttt{xarray} includes metadata in their data type, it does consider inherently features with gradients~\cite{hoyer2017xarray}.
...
%Furthermore, in atomistic learning tensor products and contraction operations are extensively used that act on different parts of the feature space, such as the atomic species, neighbors, equivariances, and environments.
By incorporating metadata into the data type, these operations can be implemented more generically and thus be reused for multiple model implementations.
%the model ne, requires an understanding of the decomposition of the target property into local contributions Equation~\eqref{eq:structural_separation} as well as an understanding of the relationship between features and gradients.
...
%%implementation of a serialization for the model plus interface for a MD package.
